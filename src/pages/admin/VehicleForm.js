    ...
    <div className="p-8">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              ...
              {models.filter(m => m.brandName === formData.make).length > 0 ? (
                ...
                <input
                  ...
                  placeholder={formData.make ? `Enter ${formData.make} model` : 'Select make first'}
                  disabled={!formData.make}
                  className="w-full px-4 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-900 disabled:opacity-50"
                />
              )}
            </div>
            <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-600 mb-2">Images (max 10)</label>
            <ImageUpload
              vehicleId={id}
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {uploadedImages.map((url, idx) => (
                  <div key={idx} className="relative">
                    ...
                    <button type="button" onClick={() => setFormData((prev) => ({ ...prev, coverImage: url }))} className="absolute bottom-1 left-1 text-xs bg-blue-600 text-white px-2 py-1 rounded">Cover</button>
